...
const CourseForm = ({ isOpen, onClose, onSubmit, instructors, course }: CourseFormProps) => {
  ...
  const courseCompleted = isEditing && new Date(form.endDate).getTime() <= Date.now();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Xatolik toasti allaqachon ko'rsatiladi
    }
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} size="xl">
      <ModalOverlay />
      <ModalContent>
        <ModalHeader>{isEditing ? "Kursni tahrirlash" : "Yangi kurs qo'shish"}</ModalHeader>
        <ModalCloseButton />
        <ModalBody>
          <Stack spacing={4}>
            ...
            <FormControl isDisabled={courseCompleted}>
              ...
              {courseCompleted && (
                <FormHelperText color="gray.500">
                  Kurs yakunlangan Ч instruktorga o'zgartirish kiritib bo'lmaydi.
                </FormHelperText>
              )}
            </FormControl>
          </Stack>
    ...
    <Stack spacing={8}>
      ...
      >
        <Box>
          ...
          <Text color="gray.500">Kurslarni qo'shish, tahrirlash va holatini kuzatish.</Text>
        </Box>
